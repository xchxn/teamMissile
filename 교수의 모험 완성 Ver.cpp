...
void UpdateGame() {
   ...
   if(character.score >= 1800 && character.score <= 3000 && boss_tick1 < tick)      //스코어 1800이상 되면 첫번? 보스 출현 kind 400 
   {
      Create_Object(MAP_X_MAX-22 ,9,400);
      boss_tick1 += 100000000;
   }
   
   if(character.score >= 6500 && character.score <= 8000 && boss_tick2 < tick && clear_boss == 1)      //스코어 6500이상 되면 두번? 보스 출현 kind 401 
   {
...
{	
	...
	if (character.hp[1] < 1)
 	...
 	//깜빡임 
	if (character.tick[3] > 0)		//몬스터와 충돌이 나면 character.tick[3] = 100이 ?. 
	{
	...
	//캐릭터 그리는 부분 
	if (character.tick[3] % 2 == 0) {		//무적tick이 짝수면 (무적tick이 100에서 1씩 계속 줄어듬. 따라서 캐릭터가 깜빡이게 ?)
...
void Control_Particle(int index) {    
   ...
   if (objects[index]->kind == 200) { 
      if (objects[index]->tick[1] < tick) {
         objects[index]->tick[1] = tick * 2;      //if문이 계속 실행되면 가속도값이 계속 초기화 되므로 동전이 점프를 하게?... 따라서 다시 실행 안되도록 tick값 설정 
