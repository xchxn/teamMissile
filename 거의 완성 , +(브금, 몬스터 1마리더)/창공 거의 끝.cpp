...
typedef struct _Object {   //enemies, projectiles, particles, etc.
    ...
    int tick[4];   //[0]: hpshow time(enemy)  [1] : 동전 가속도 딜레이(한번만 발생하도록)
}Object;
...
//몬스터 모양 
const char figure_enemy1[2][13] = {" __ (++)----", " __ [oo]\'--\'"};
//보스 거미 모양 
...
void Create_Object(int x, int y, int kind);      //x,y좌표에 kind값에 따라 오브젝트 생성 (1~99: 아이템, 100~199: 몬스터, 200~299: 동전)
...
void UpdateGame() {
   ...
   if(character.score >= 6500 && character.score <= 8000 && boss_tick < tick+100000000)      //스코어 6500이상 되면 첫번쨰 보스 출현 kind 401
   {
   ...
   for(int i = 0; i < OBJECT_MAX; i++) {
      if (objects[i]) {
         if (objects[i]->kind < 100)   //0~99는 아이템 
            Control_Item(i);
         else if ((objects[i]->kind > 99 && objects[i]->kind < 200) || objects[i]->kind == 401 || objects[i]->kind == 400 || objects[i]->kind == 501)    //100~199는 슬라임, 400,401은 보스 
            Control_Enemy(i);
      
         else
            ControlParticle(i);      //나머지는 동전(00) 및 무기(1,2) 
      }
   }
}
...
void Create_Object(int x, int y, int kind) {      //x,y좌표에 kind값에 따라 오브젝트 생성 (1~99: 아이템, 100~199: 몬스터, 200~299: 동전)
   ...
   obj = (Object *)malloc(sizeof(Object));    //obj가 새로운 object를 가리킴  
    ...
    if ((kind < 100 || kind > 199) && kind <502 ) {      //100미만 200이상  -->  몬스터아님  
   ...
   //Boss spider = {{MAP_X_MAX-22,6},{18,20},{500,500},1000}; //x,y  sizex,sizey hpmax,hp, exp
